...
def create_new_document(sent_list:List[Sentence]) -> Tuple[str, List[Sentence]]:
    '''create new source document
    '''
    document = ''
    new_sent_list = []
    for sent in sent_list:
        new_sent_list.append(Sentence(int(len(document)), sent.text))
        document += sent.text + ' '
    for sent in new_sent_list: # check
        assert document[sent.start:sent.start+len(sent.text)] == sent.text
    return document.strip(), new_sent_list

def find_event_sent(event_start, trigger, sent_list) -> Tuple[int, int]:
...
def xml_parser(file_path:str, sent_list:List[Sentence]) -> Dict:
    ...
    document, new_sent_list = create_new_document(sent_list)
    new_sentence_list = [{'start': sent.start, 'text': sent.text} for sent in new_sent_list]
    event_list = []
    cluster_list = []
    for hopper in tree.iter(tag='hopper'):
        ...
        for event in hopper.iter(tag='event_mention'):
            ...
            e_s_index, e_s_start = find_event_sent(int(event.find('trigger').attrib['offset']), e_trigger, sent_list)
            e_start = new_sent_list[e_s_index].start + e_s_start
            e_trigger = update_trigger(new_sent_list[e_s_index].text, e_trigger, e_s_start)
            assert document[e_start:e_start+len(e_trigger)] == e_trigger
            event_list.append({
                'event_id': e_id, 
                'start': e_start, 
                'trigger': e_trigger, 

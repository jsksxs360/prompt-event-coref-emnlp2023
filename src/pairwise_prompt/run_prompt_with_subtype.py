...
from src.pairwise_prompt.data import SUBTYPES
...
from src.pairwise_prompt.modeling import BertForPromptwithSubtype, RobertaForPromptwithSubtype, LongformerForPromptwithSubtype
...
MODEL_CLASSES = {
    'bert': BertForPromptwithSubtype,
    'roberta': RobertaForPromptwithSubtype, 
    'longformer': LongformerForPromptwithSubtype
}
PROMPT_LENGTH = {
    'hbt_d': 40, 'd_hbt': 40,  # hard base template
    'hqt_d': 40, 'd_hqt': 40,  # hard question-style template
    'sbt_d': 40, 'd_sbt': 40   # soft base template
}
CONTEXT_K = 2
...
def predict(args, model, tokenizer, 
    ...
    t1_mask_idx = prompt_data['tmp_t1_idx']
    t2_mask_idx = prompt_data['tmp_t2_idx']
    ...
    inputs = {
        'batch_inputs': inputs, 
        'batch_t1_mask_idx': [t1_mask_idx], 
        'batch_t2_mask_idx': [t2_mask_idx], 
...
if __name__ == '__main__':
    ...
    subtype_descriptions = ['normal'] + SUBTYPES
    with torch.no_grad():
        for i, token in enumerate(reversed(subtype_descriptions), start=1):
    ...
    if args.do_train:
        ...
        train(args, train_dataset, dev_dataset, model, tokenizer, 
            add_mark=args.model_type, collote_fn_type='with_subtype', prompt_type=args.prompt_type, verbalizer=verbalizer
        )
    # Testing

...
from src.trigger_detection.modeling import LongformerCRFForTD, BigBirdCRFForTD
...
MODEL_CLASSES = {
    'longformer': LongformerCRFForTD, 
    'bigbird': BigBirdCRFForTD
}
...
def train(args, train_dataset, dev_dataset, model, tokenizer):
    ...
    longformer_param_optimizer = list(model.longformer.named_parameters())
    crf_param_optimizer = list(model.crf.named_parameters())
    linear_param_optimizer = list(model.classifier.named_parameters())
    optimizer_grouped_parameters = [
        {'params': [p for n, p in longformer_param_optimizer if not any(nd in n for nd in no_decay)],
         'weight_decay': args.weight_decay, 'lr': args.learning_rate},
        {'params': [p for n, p in longformer_param_optimizer if any(nd in n for nd in no_decay)], 'weight_decay': 0.0,
...
def predict(args, document:str, model, tokenizer):
    ...
    inputs = to_device(args, args.device)
    with torch.no_grad():
